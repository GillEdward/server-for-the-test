    ...
    ws.on("message", function(data){    // 处理客户端发送过来的消息 // on message
        ...
        switch(json["action"]){
            case "login":
                ...
                wss.broadcast(JSON.stringify({"player-id" : json["player-id"], "action" : "newPlayerLogin", "value" : initPos}))    // 广播新玩家的ID和pos // broadcast new player's ID and pos
                break
            case "logout":
            ...
            case "moving":
                if(json["value"] & 0b1000){ // Left
                    playerList[json["player-id"]][0] -= 1   // 修改服务器本地坐标    // change local pos of the player
                }
                ...
                break
            case "movementDone":
                wss.broadcast(JSON.stringify({"player-id" : json["player-id"], "action" : "movementDone", "value" : playerList[json["player-id"]]}))    // 广播停止运动的玩家的标准坐标   // broadcast the player that stop moving, sycn every player's playerList
                break
        }
    })
...
wss.broadcast = function broadcast(msg) {   // broadcast function
